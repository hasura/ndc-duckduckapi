...
): SchemaDerivationResults {
  const programResult = createTsProgram(functionsFilePath);
  if (programResult instanceof Ok) {
    const { program, compilerDiagnostics, ndcLambdaSdkModule } =
      programResult.data;
    const sourceFile = program.getSourceFile(functionsFilePath);
    const projectRootDir = getProjectRootDirectory(functionsFilePath);
    if (!sourceFile)
      throw new Error(
        `'${functionsFilePath}' not returned as a TypeScript compiler source file`,
      );
    const [functionsSchema, functionIssues] = deriveSchemaFromFunctions(
      sourceFile,
      projectRootDir,
...
function createTsProgram(
  functionsFilePath: string,
): Result<CompilerResults, ts.Diagnostic[]> {
  return loadTsConfig(functionsFilePath).bind((parsedCommandLine) => {
    ...
    const sdkModule = ts.resolveModuleName(
    ...
    return compilerDiagnostics.find(
      (d) => d.category === ts.DiagnosticCategory.Error,
    ) === undefined
      ? new Ok({
          program,
          compilerDiagnostics: [...compilerDiagnostics],
          ndcLambdaSdkModule: sdkModule.resolvedModule,
        })
...
function loadTsConfig(
  functionsFilePath: string,
): Result<ts.ParsedCommandLine, ts.Diagnostic[]> {
  ...
  const fallbackTsConfig = path.resolve(
...
function getProjectRootDirectory(functionsFilePath: string): string {
  ...
  while (true) {
    ...
    if (parentDir === currentDir) return path.dirname(functionsFilePath);
...
function cloneTypeDerivationContext(
  context: TypeDerivationContext,
): TypeDerivationContext {
  return {
...
function deriveSchemaFromFunctions(
  sourceFile: ts.SourceFile,
  projectRootDir: string,
  typeChecker: ts.TypeChecker,
  ndcLambdaSdkModule: ts.ResolvedModuleFull,
): [schema.FunctionsSchema, FunctionIssues] {
  ...
  const sourceFileSymbol =
    ...
    throwError("sourceFile does not have a symbol");
  const functionDeclarations: [string, ts.FunctionDeclaration][] = typeChecker
    .getExportsOfModule(sourceFileSymbol)
    .flatMap((exportedSymbol) => {
      const declaration =
        exportedSymbol.getDeclarations()?.[0] ??
        throwError("exported symbol does not have a declaration");
      ...
      if (ts.isExportSpecifier(declaration)) {
        const identifier =
          declaration.name ??
          throwError("export declaration didn't have an identifier");
        const exportTarget =
          typeChecker.getExportSpecifierLocalTargetSymbol(declaration) ??
          throwError("export specifier does not have a local target symbol");
      ...
      // If just a plain function export or exported via 'export * from "./imported"'
        ...
        const identifier =
          declaration.name ??
          throwError("function declaration didn't have an identifier");
  ...
  for (const [
    ...
    functionDeclaration,
  ] of functionDeclarations) {
    const result = deriveFunctionSchema(
      functionDeclaration,
      exportedFunctionName,
      typeDerivationContext,
    );
  ...
  const finalTypeNames =
    typeDerivationContext.customTypeNameRegistry.determineFinalTypeNames(
      projectRootDir,
      typeChecker,
    );
  const finalFunctionSchema = applyFinalTypeNamesToFunctionsSchema(
    functionsSchema,
    finalTypeNames,
  );
...
function deriveFunctionSchema(
  functionDeclaration: ts.FunctionDeclaration,
  exportedFunctionName: string,
  context: TypeDerivationContext,
): Result<schema.FunctionDefinition, string[]> {
  const functionIdentifier =
    functionDeclaration.name ??
    throwError("Function didn't have an identifier");
  const functionSymbol =
    context.typeChecker.getSymbolAtLocation(functionIdentifier) ??
    throwError(`Function '${exportedFunctionName}' didn't have a symbol`);
  const functionType = context.typeChecker.getTypeOfSymbolAtLocation(
    functionSymbol,
    functionDeclaration,
  );

  const functionDescription = getDescriptionFromJsDoc(
    functionSymbol,
    context.typeChecker,
  );
  const markedReadonlyInJsDoc =
    functionSymbol.getJsDocTags().find((e) => e.name === "readonly") !==
    undefined;
  const allowRelaxedTypes =
    functionSymbol
      .getJsDocTags()
      .find((e) => e.name === "allowrelaxedtypes") !== undefined;
  const parallelDegreeResult = getParallelDegreeFromJsDoc(
    functionSymbol,
    markedReadonlyInJsDoc,
  );

  const functionCallSig =
    functionType.getCallSignatures()[0] ??
    throwError(
      `Function '${exportedFunctionName}' didn't have a call signature`,
    );
  const functionSchemaArguments: Result<schema.ArgumentDefinition[], string[]> =
    Result.traverseAndCollectErrors(
      functionCallSig.getParameters(),
      (paramSymbol) => {
        const paramName = paramSymbol.getName();
        const paramDesc = ts
          .displayPartsToString(
            paramSymbol.getDocumentationComment(context.typeChecker),
          )
          .trim();
        const paramType = context.typeChecker.getTypeOfSymbolAtLocation(
          paramSymbol,
          paramSymbol.valueDeclaration ??
            throwError(
              `Function '${exportedFunctionName}' parameter '${paramName}' didn't have a value declaration`,
            ),
        );
        const paramTypePath: schema.TypePathSegment[] = [
          {
        ...
        return deriveSchemaTypeForTsType(
          paramType,
          { type: "Symbol", symbol: paramSymbol },
          paramTypePath,
          allowRelaxedTypes,
          context,
        ).map((paramTypeResult) => ({
  ...
  const unwrappedReturnType =
    unwrapPromiseType(returnType, context.typeChecker) ?? returnType;
  const returnTypeResult = deriveSchemaTypeForTsType(
    unwrappedReturnType,
    { type: "FunctionReturn", callSignature: functionCallSig },
    [{ segmentType: "FunctionReturn", functionName: exportedFunctionName }],
    allowRelaxedTypes,
    context,
  );

  return Result.collectErrors3(
    functionSchemaArguments,
    returnTypeResult,
    parallelDegreeResult,
  ).map(([functionSchemaArgs, returnType, parallelDegree]) => ({
    description: functionDescription,
    ndcKind: markedReadonlyInJsDoc
      ? schema.FunctionNdcKind.Function
      : schema.FunctionNdcKind.Procedure,
...
function getDescriptionFromJsDoc(
  symbol: ts.Symbol,
  typeChecker: ts.TypeChecker,
): string | null {
...
function getParallelDegreeFromJsDoc(
  functionSymbol: ts.Symbol,
  functionIsReadonly: boolean,
): Result<number | null, string[]> {
  const parallelDegreeTag = functionSymbol
    .getJsDocTags()
    .find((e) => e.name === "paralleldegree");
  if (parallelDegreeTag === undefined) {
    ...
    if (tagSymbolDisplayPart === undefined)
      return new Err([
        "The @paralleldegree JSDoc tag must specify an integer degree value",
      ]);
    ...
    if (isNaN(parallelDegreeInt) || parallelDegreeInt <= 0)
      return new Err([
        `The @paralleldegree JSDoc tag must specify an integer degree value that is greater than 0. Current value: '${tagText}'`,
      ]);
...
function deriveSchemaTypeForTsType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  context: TypeDerivationContext,
): Result<schema.TypeReference, string[]> {
  const typeRenderedName = context.typeChecker.typeToString(tsType);

  if (unwrapPromiseType(tsType, context.typeChecker) !== undefined) {
    return new Err([
      `Promise types are not supported, but one was encountered in ${schema.typePathToString(typePath)}.`,
    ]);
  }

  if (tsutils.isIntrinsicVoidType(tsType)) {
    return new Err([
      `The void type is not supported, but one was encountered in ${schema.typePathToString(typePath)}`,
    ]);
  }

  if (tsutils.isIntrinsicNeverType(tsType)) {
    return new Err([
      `The never type is not supported, but one was encountered in ${schema.typePathToString(typePath)}`,
    ]);
  }

  if (tsutils.isIntrinsicNonPrimitiveType(tsType)) {
    return new Err([
      `The object type is not supported, but one was encountered in ${schema.typePathToString(typePath)}`,
    ]);
  }

  if (tsutils.isIntrinsicNullType(tsType)) {
    return new Err([
      `The null type is not supported as a type literal used on its own, but one was encountered in ${schema.typePathToString(typePath)}`,
    ]);
  }

  if (tsutils.isIntrinsicUndefinedType(tsType)) {
    return new Err([
      `The undefined type is not supported as a type literal used on its own, but one was encountered in ${schema.typePathToString(typePath)}`,
    ]);
  }

  if (isFunctionType(tsType)) {
    return new Err([
      `Function types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${context.typeChecker.typeToString(tsType)})`,
    ]);
  }

  if (isMapType(tsType)) {
    return new Err([
      `Map types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${context.typeChecker.typeToString(tsType)})`,
    ]);
  }

  const schemaTypeResult =
    deriveSchemaTypeIfTsUnknownOrAny(
      tsType,
      typeSource,
      typePath,
      allowRelaxedTypes,
    ...
    deriveSchemaTypeIfNullableType(
      tsType,
      typeSource,
      typePath,
      allowRelaxedTypes,
    ...
    deriveSchemaTypeIfTsIndexSignatureType(
      tsType,
      typeSource,
      typePath,
      allowRelaxedTypes,
      context,
    ) ?? // This needs to be done after scalars and classes, etc because some of those types do have index signatures (eg. strings)
    deriveSchemaTypeIfTsUnionType(
      tsType,
      typeSource,
      typePath,
      allowRelaxedTypes,
      context,
    ); // This needs to be done after nullable types, since nullable types use unions, this catches everything else

  if (schemaTypeResult !== undefined) return schemaTypeResult;

  // We don't know how to deal with this type, so reject it with a generic error
  return new Err([
    `Unable to derive an NDC type for ${schema.typePathToString(typePath)} (type: ${context.typeChecker.typeToString(tsType)}).`,
  ]);
}

function deriveRelaxedTypeOrError(
  ...
  const uniqueTypeIdentifier = makeUniqueTypeIdentifier(
    tsType,
    typeSource,
    context.typeChecker,
  );
  context.customTypeNameRegistry.registerUniqueType(
  ...
  let scalarTypeDefinition =
    context.scalarTypeDefinitions[uniqueTypeIdentifier];
  if (scalarTypeDefinition === undefined) {
    ...
    throw new Error(
      `Scalar type name conflict. Trying to create relaxed type '${uniqueTypeIdentifier}' but it already exists as a ${scalarTypeDefinition.type}`,
    );
  }
...
function deriveSchemaTypeIfTsUnknownOrAny(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  context: TypeDerivationContext,
): Result<schema.TypeReference, string[]> | undefined {
  if (tsutils.isIntrinsicUnknownType(tsType)) {
    return deriveRelaxedTypeOrError(
      "unknown",
      tsType,
      typeSource,
      typePath,
      () =>
        `The unknown type is not supported, but one was encountered in ${schema.typePathToString(typePath)}`,
      allowRelaxedTypes,
      context,
    );
  }
  ...
  if (tsutils.isIntrinsicAnyType(tsType)) {
    return deriveRelaxedTypeOrError(
      ...
      typePath,
      () =>
        `The any type is not supported, but one was encountered in ${schema.typePathToString(typePath)}`,
      allowRelaxedTypes,
      context,
    );
  }
...
function deriveSchemaTypeIfTsTupleType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (tsutils.isTupleTypeReference(tsType)) {
    ...
    if (allowRelaxedTypes) {
      ...
      const result = Result.traverseAndCollectErrors(
        tsType.typeArguments ?? [],
        (typeParameterTsType: ts.Type, index: number) => {
          return deriveSchemaTypeForTsType(
            typeParameterTsType,
            typeSource,
            [...typePath, { segmentType: "TypeParameter", typeName, index }],
            allowRelaxedTypes,
            isolatedContext,
          );
        },
    ...
    return deriveRelaxedTypeOrError(
      ...
      typePath,
      () =>
        `Tuple types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${typeName})`,
      allowRelaxedTypes,
      context,
    );
  }
...
function rejectIfClassType(
  tsType: ts.Type,
  typePath: schema.TypePathSegment[],
  context: TypeDerivationContext,
): Result<schema.TypeReference, string[]> | undefined {
  if (
    tsutils.isObjectType(tsType) &&
    tsutils.isObjectFlagSet(tsType, ts.ObjectFlags.Class)
  ) {
    return new Err([
      `Class types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${context.typeChecker.typeToString(tsType)})`,
    ]);
  }
}

// Types with index signatures: ie '[x: T]: Y'
function deriveSchemaTypeIfTsIndexSignatureType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (indexInfos.length > 0) {
    ...
    if (allowRelaxedTypes) {
      ...
      const isolatedContext = cloneTypeDerivationContext(context); // Use an isolated context so we don't actually record any new scalar or object types while doing this, since this is going to end up as a relaxed type anyway
      const indexSignatureTypes = indexInfos.flatMap<
        [ts.Type, schema.TypePathSegment]
      >((indexInfo: ts.IndexInfo, sigIndex: number) => [
        [
          indexInfo.keyType,
          {
    ...
    return deriveRelaxedTypeOrError(
      ...
      typePath,
      () =>
        `Types with index signatures are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${typeName})`,
      allowRelaxedTypes,
      context,
    );
  }
...
function deriveSchemaTypeIfTsUnionType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (tsType.isUnion()) {
    ...
    if (allowRelaxedTypes) {
      ...
      const result = Result.traverseAndCollectErrors(
        tsType.types,
        (memberTsType: ts.Type, memberIndex: number) => {
          return deriveSchemaTypeForTsType(
            memberTsType,
            typeSource,
            [
              ...typePath,
              { segmentType: "UnionMember", typeName, memberIndex },
            ],
            allowRelaxedTypes,
            isolatedContext,
          );
    ...
    return deriveRelaxedTypeOrError(
      ...
      typePath,
      () =>
        `Union types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${typeName})`,
      allowRelaxedTypes,
      context,
    );
  }
...
function deriveSchemaTypeIfTsArrayType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (
    context.typeChecker.isArrayType(tsType) &&
...
function deriveSchemaTypeIfBuiltInScalarType(
  tsType: ts.Type,
  context: TypeDerivationContext,
): Result<schema.TypeReference, string[]> | undefined {
  if (tsutils.isIntrinsicBooleanType(tsType) || isBooleanUnionType(tsType)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.Boolean,
    });
  }
  ...
  if (tsutils.isIntrinsicStringType(tsType)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.String,
    });
  }
  ...
  if (tsutils.isIntrinsicNumberType(tsType)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.Float,
    });
  }
  ...
  if (tsutils.isIntrinsicBigIntType(tsType)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.BigInt,
    });
  }
  ...
  if (isDateType(tsType)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.DateTime,
    });
  }
  if (isJSONValueType(tsType, context.ndcLambdaSdkModule)) {
    ...
    return new Ok({
      type: "named",
      kind: "scalar",
      name: schema.BuiltInScalarTypeName.JSON,
    });
  }
}

function deriveSchemaTypeIfEnumType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (
    tsutils.isUnionType(tsType) &&
    !tsutils.isIntrinsicType(tsType) /* Block booleans */
  ) {
    const typeName = context.typeChecker.typeToString(tsType);
    ...
    if (tsutils.isTypeFlagSet(tsType, ts.TypeFlags.EnumLiteral)) {
      return deriveRelaxedTypeOrError(
        ...
        typePath,
        () =>
          `Enum types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${typeName})`,
        allowRelaxedTypes,
        context,
      );
    }
    ...
    if (
      tsType.types.every((unionMemberType) =>
        tsutils.isLiteralType(unionMemberType),
      )
    ) {
      return deriveRelaxedTypeOrError(
        typeName,
        ...
        typePath,
        () =>
          `Literal union types are not supported, but one was encountered in ${schema.typePathToString(typePath)} (type: ${typeName})`,
        allowRelaxedTypes,
        context,
      );
    }
  ...
  // Handles computed single member enum types: 'enum { OneThing = "test".length }'
    ...
    tsutils.isEnumType(tsType) &&
    tsutils.isSymbolFlagSet(tsType.symbol, ts.SymbolFlags.EnumMember)
  ) {
    const typeName = context.typeChecker.typeToString(tsType);
    return deriveRelaxedTypeOrError(
      typeName,
...
function isDateType(tsType: ts.Type): boolean {
  ...
  return (
    ...
    symbol.members?.has(ts.escapeLeadingUnderscores("toISOString")) === true
  );
}
...
function isMapType(tsType: ts.Type): tsType is ts.TypeReference {
  ...
  return (
    ...
    symbol.members?.has(ts.escapeLeadingUnderscores("keys")) === true &&
    symbol.members?.has(ts.escapeLeadingUnderscores("values")) === true &&
    symbol.members?.has(ts.escapeLeadingUnderscores("entries")) === true
  );
}
...
function isBooleanUnionType(tsType: ts.Type): boolean {
  ...
  return (
    tsType.types.length === 2 &&
    tsType.types.find(
      (type) =>
        tsutils.isBooleanLiteralType(type) && type.intrinsicName === "true",
    ) !== undefined &&
    tsType.types.find(
      (type) =>
        tsutils.isBooleanLiteralType(type) && type.intrinsicName === "false",
    ) !== undefined
  );
}
...
function deriveSchemaTypeIfNullableType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (notNullableResult !== null) {
    ...
    return deriveSchemaTypeForTsType(
      ...
      type: "nullable",
      underlyingType: notNullableType,
      nullOrUndefinability,
    }));
  }
}

function deriveSchemaTypeIfObjectType(
  tsType: ts.Type,
  typeSource: TypeSource,
  typePath: schema.TypePathSegment[],
  allowRelaxedTypes: boolean,
  ...
  if (info) {
    const makeRelaxedTypesError: () => Result<
      schema.TypeReference,
      string[]
    > = () =>
      new Err<schema.TypeReference, string[]>([
        `The object type '${info.preferredTypeName}' uses relaxed types and can only be used by a function marked with @allowrelaxedtypes. It was encountered in ${schema.typePathToString(typePath)}`,
      ]);

    // Short-circuit recursion if the type has already been named
    const existingType =
      context.objectTypeDefinitions[info.uniqueTypeIdentifier];
    if (existingType) {
      ...
      return new Ok({
        type: "named",
        name: info.uniqueTypeIdentifier,
        kind: "object",
      });
    }
    ...
    return Result.traverseAndCollectErrors(
        ...
        return deriveSchemaTypeForTsType(
          ...
          [
            ...typePath,
            {
              ...
              typeName: context.typeChecker.typeToString(tsType),
              preferredTypeName: info.preferredTypeName,
              propertyName,
            },
          ],
          allowRelaxedTypes,
      ...
      .bind((propertyResults) => {
        const isRelaxedType = propertyResults.some((propDef) =>
          isTypeReferenceARelaxedType(propDef.type, context),
        );
        ...
        context.customTypeNameRegistry.registerUniqueType(
          info.uniqueTypeIdentifier,
          tsType,
          typeSource,
          info.preferredTypeName,
        );
...
function isTypeReferenceARelaxedType(
  typeReference: schema.TypeReference,
  context: TypeDerivationContext,
): boolean {
  switch (typeReference.type) {
    ...
    case "named":
      switch (typeReference.kind) {
        case "object":
          const objTypeDef =
            context.objectTypeDefinitions[typeReference.name] ??
            throwError(
              `Unable to find object type '${typeReference.name}', which should have already been generated`,
            );
          return objTypeDef.isRelaxedType;
        case "scalar":
          const scalarTypeDef =
            context.scalarTypeDefinitions[typeReference.name] ??
            throwError(
              `Unable to find object type '${typeReference.name}', which should have already been generated`,
            );
...
function unwrapPromiseType(
  ...
  if (
    tsutils.isTypeReference(tsType) &&
    tsType.getSymbol()?.getName() === "Promise"
  ) {
    const typeArgs = typeChecker.getTypeArguments(tsType);
    return typeArgs.length === 1 ? typeArgs[0] : undefined; // This isn't a real Promise if it doesn't have exactly one type argument
  } else {
    return undefined; // Not a Promise
  }
}

function unwrapNullableType(
  tsType: ts.Type,
  typeChecker: ts.TypeChecker,
): [ts.Type, schema.NullOrUndefinability] | null {
  if (!tsType.isUnion()) return null;

  const isNullable =
    tsType.types.find(tsutils.isIntrinsicNullType) !== undefined;
  const isUndefined =
    tsType.types.find(tsutils.isIntrinsicUndefinedType) !== undefined;
...
function isAnonymousObjectType(tsType: ts.Type): boolean {
  return (
    tsutils.isObjectType(tsType) &&
    tsutils.isObjectFlagSet(tsType, ts.ObjectFlags.Anonymous)
  );
}
...
function isInterfaceType(tsType: ts.Type): boolean {
  return (
    tsutils.isObjectType(tsType) &&
    tsutils.isObjectFlagSet(tsType, ts.ObjectFlags.Interface)
  );
}
...
function isGenericInterfaceType(
  tsType: ts.Type,
  typeChecker: ts.TypeChecker,
): boolean {
  return (
    tsutils.isTypeReference(tsType) &&
    tsutils.isObjectFlagSet(tsType.target, ts.ObjectFlags.Interface) &&
    typeChecker.isArrayType(tsType) === false &&
    tsType.getSymbol()?.getName() !== "Promise"
  );
}
...
function getObjectTypeInfo(
  ...
  if (
    !isAnonymousObjectType(tsType) &&
    !isInterfaceType(tsType) &&
    !isGenericInterfaceType(tsType, typeChecker) &&
    !tsutils.isIntersectionType(tsType)
  ) {
    return null;
  }
  ...
  return {
    uniqueTypeIdentifier: makeUniqueTypeIdentifier(
      tsType,
      typeSource,
      typeChecker,
    ),
    preferredTypeName:
      ...
      (isAnonymousObjectType(tsType) || tsutils.isIntersectionType(tsType)) &&
      !tsType.aliasSymbol
    ...
    description: symbolForDocs
      ? getDescriptionFromJsDoc(symbolForDocs, typeChecker)
      : null,
  };
}
...
function generateTypeNameFromTypePath(
  typePath: schema.TypePathSegment[],
): string {
  ...
  switch (lastSegment.segmentType) {
    ...
    case "FunctionParameter":
      return `${lastSegment.functionName}_${lastSegment.parameterName}`;
    case "FunctionReturn":
      return `${lastSegment.functionName}_output`;
    case "ObjectProperty":
      return `${lastSegment.preferredTypeName}_${lastSegment.propertyName}`;

    case "Array":
      return `array`;
    case "TypeParameter":
      return `typeparam_${lastSegment.index}`;
    case "IndexSignature":
      return `indexsig_${lastSegment.sigIndex}_${lastSegment.segmentType}`;
    case "UnionMember":
      return `union_${lastSegment.memberIndex}`;
    default:
      return unreachable(lastSegment["segmentType"]);
  }
}
...
class CustomTypeNameRegistry {
  ...
  _preferredTypeNameContenders: Record<
    PreferredTypeName,
    UniqueTypeIdentifier[]
  > = {};
  ...
  registerUniqueType(
    ...
    if (this._uniqueTypes[uniqueTypeIdentifier] !== undefined) return; // Already registered

    this._uniqueTypes[uniqueTypeIdentifier] = {
      preferredName: preferredTypeName,
      typeSource,
      tsType,
    };
    const contenders =
      this._preferredTypeNameContenders[preferredTypeName] ?? [];
    contenders.push(uniqueTypeIdentifier);
    this._preferredTypeNameContenders[preferredTypeName] = contenders;
  }

  determineFinalTypeNames(
    projectRootDir: string,
    typeChecker: ts.TypeChecker,
  ): Record<UniqueTypeIdentifier, string> {
    const usedNames: Set<string> = new Set(
      Object.values(schema.BuiltInScalarTypeName),
    ); // Built-in names are reserved already
    ...
    const allocateName = (name: string) => {
      ...
      for (let index = 2; usedNames.has(proposedName); index++) {
        if (index >= Number.MAX_SAFE_INTEGER)
          throw new Error(
            `Unable to find an unused name for '${name}'. Reached MAX_SAFE_INTEGER`,
          );
    ...
    return mapObjectValues(this._uniqueTypes, (typeInfo, uniqueTypeName) => {
      const preferredNameContenders =
        this._preferredTypeNameContenders[typeInfo.preferredName] ?? [];
      const name =
        preferredNameContenders.length === 1
  ...
  clone(): CustomTypeNameRegistry {
    ...
    cloned._uniqueTypes = mapObjectValues(this._uniqueTypes, (typeInfo) => ({
      ...typeInfo,
    }));
    cloned._preferredTypeNameContenders = mapObjectValues(
      this._preferredTypeNameContenders,
 ...
 */
function makeUniqueTypeIdentifier(
  tsType: ts.Type,
  typeSource: TypeSource,
  typeChecker: ts.TypeChecker,
): UniqueTypeIdentifier {
  ...
  type TypeId =
    | TypeWithDeclarationId
    | IntrinsicTypeId
    | LiteralTypeId
    | AnonymousObjectTypeId
    | AnonymousUnionTypeId
    | AnonymousIntersectionTypeId;
  type TypeWithDeclarationId = { t: "d"; f: string; s: number; ta: TypeId[] };
  type IntrinsicTypeId = { t: "i"; i: string };
  type LiteralTypeId =
    | { t: "l-n"; v: number }
    | { t: "l-s"; v: string }
    | { t: "l-bi"; v: ts.PseudoBigInt };
  ...
  const compareTypeId = (typeIdA: TypeId, typeIdB: TypeId): number => {
    if (typeIdA.t === "d" && typeIdB.t == "d") {
      return (
        typeIdA.f.localeCompare(typeIdB.f) ||
        typeIdA.s - typeIdB.s ||
      ...
      return compareList(
        ([nameA, pTypeIdA], [nameB, pTypeIdB]) =>
          nameA.localeCompare(nameB) || compareTypeId(pTypeIdA, pTypeIdB),
        typeIdA.p,
        typeIdB.p,
      );
  ...
  const deriveTypeId = (tsType: ts.Type): TypeId => {
    ...
    if (!tsType.aliasSymbol && isAnonymousObjectType(tsType)) {
      return {
        ...
        p: tsType
          .getProperties()
          .map<[string, TypeId]>((propSymbol) => [
            propSymbol.name,
            deriveTypeId(typeChecker.getTypeOfSymbol(propSymbol)),
          ])
    ...
    if (!tsType.aliasSymbol && tsType.isIntersection()) {
      return {
        t: "in",
        in: tsType.types.map(deriveTypeId).sort(compareTypeId),
      }; // We sort the types to ensure 'TypeA & TypeB' is the same as 'TypeB & TypeA'
    }
    const declaration = getDeclarationFromTypeOrTypeSource(
      tsType,
      typeSource,
      typeChecker,
    );
    const typeArguments =
      ...
      (tsutils.isTypeReference(tsType) ? tsType.typeArguments : undefined) ??
      [];
...
function getDeclarationFromTypeOrTypeSource(
  tsType: ts.Type,
  typeSource: TypeSource,
  typeChecker: ts.TypeChecker,
): ts.Declaration {
  ...
  if (symbolFromType) {
    return (
      symbolFromType.getDeclarations()?.[0] ??
      throwError(
        `Couldn't find declaration of symbol for type '${typeChecker.typeToString(tsType)}'`,
      )
    );
  } else {
    ...
    switch (typeSource.type) {
      ...
      case "Symbol":
        return (
          typeSource.symbol.getDeclarations()?.[0] ??
          throwError(
            `Couldn't find declaration of type source symbol for type '${typeChecker.typeToString(tsType)}'`,
          )
        );
      default:
...
function deriveFullyQualifiedName(
  uniqueTypeInfo: UniqueTypeInfo,
  projectRootDir: string,
  typeChecker: ts.TypeChecker,
): string {
  const declaration = getDeclarationFromTypeOrTypeSource(
    uniqueTypeInfo.tsType,
    uniqueTypeInfo.typeSource,
    typeChecker,
  );
  ...
  const shortenedFileName =
    ...
    fileName;
  const moduleNameComponent = dropFileExtension(shortenedFileName).replaceAll(
    /[\\\/]/g,
    "_",
  );
...
function stripNodeModulesDirectory(filepath: string): string | null {
  ...
  return lastIndex !== null ? filepath.substring(lastIndex) : null;
}

function stripProjectRootPath(
  filepath: string,
  projectRootDir: string,
): string | null {
  // Add a / to the end if it does not exist
  projectRootDir = projectRootDir.endsWith(path.sep)
    ? projectRootDir
    : projectRootDir + path.sep;
...
function applyFinalTypeNamesToFunctionsSchema(
  functionsSchema: schema.FunctionsSchema,
  finalTypeNames: Record<UniqueTypeIdentifier, string>,
  ...
  return {
    functions: mapObjectValues(functionsSchema.functions, (definition) => ({
      ...
      arguments: definition.arguments.map((arg) => ({
        ...arg,
        type: applyFinalTypeName(arg.type, finalTypeNames),
      })),
      resultType: applyFinalTypeName(definition.resultType, finalTypeNames),
    })),
    objectTypes: Object.fromEntries(
      Object.entries(functionsSchema.objectTypes).map(
        ([objectTypeName, definition]) => {
          const newObjectTypeName =
            finalTypeNames[objectTypeName] ??
            throwError(`Unable to find unique type name '${objectTypeName}'`);
          const newDefinition = {
            ...definition,
            properties: definition.properties.map((prop) => ({
              ...prop,
              type: applyFinalTypeName(prop.type, finalTypeNames),
            })),
          };
    ...
    scalarTypes: Object.fromEntries(
      Object.entries(functionsSchema.scalarTypes).map(
        ([scalarTypeName, definition]) => {
          const newScalarTypeName = schema.isTypeNameBuiltInScalar(
            scalarTypeName,
          )
            ? scalarTypeName // Built-in types already have their final name
            : (finalTypeNames[scalarTypeName] ??
              throwError(
                `Unable to find unique type name '${scalarTypeName}'`,
              ));
...
function applyFinalTypeName(
  ...
  switch (typeReference.type) {
    case "array":
      return {
        ...typeReference,
        elementType: applyFinalTypeName(
    ...
    case "nullable":
      return {
        ...typeReference,
        underlyingType: applyFinalTypeName(
    ...
    case "named":
      return schema.isTypeNameBuiltInScalar(typeReference.name)
        ...
        : {
            ...typeReference,
            name:
              finalTypeNames[typeReference.name] ??
              throwError(
                `Unable to find unique type name '${typeReference.name}'`,
              ),
          };
    default:
      return unreachable(typeReference["type"]);
