...
export type NamedTypeReference =
  | NamedObjectTypeReference
  | NamedScalarTypeReference;
...
export type NamedScalarTypeReference =
  | CustomNamedScalarTypeReference
  | BuiltInScalarTypeReference;

export type BuiltInScalarTypeReference =
  | StringScalarTypeReference
  | FloatScalarTypeReference
  | BooleanScalarTypeReference
  | BigIntScalarTypeReference
  | DateTimeScalarTypeReference
  | JSONScalarTypeReference;
...
function builtInScalarTypeAssertionTest(
  a: BuiltInScalarTypeReference["name"],
  b: BuiltInScalarTypeName,
): void {
  a = b;
  b = a;
}
  ...
  constructor(value: unknown, lazyValidate: boolean = false) {
    ...
    if (!lazyValidate) {
      ...
      if (
        ...
        this.#serializationError !== undefined
      ) {
        throw new Error("The provided value cannot be serialized to JSON", {
          cause: this.#serializationError,
        });
      }
    }
...
export function typePathSegmentToString(segment: TypePathSegment): string {
  switch (segment.segmentType) {
    case "FunctionParameter":
      return `function '${segment.functionName}' parameter '${segment.parameterName}'`;
    case "FunctionReturn":
      return `function '${segment.functionName}' return value`;
    case "ObjectProperty":
      return `type '${segment.typeName}' property '${segment.propertyName}'`;
    case "Array":
      return `array type`;
    case "TypeParameter":
      return `type '${segment.typeName}' type parameter index '${segment.index}'`;
    case "IndexSignature":
      return `type '${segment.typeName}' type signature index '${segment.sigIndex}' ${segment.component} type`;
    case "UnionMember":
      return `type '${segment.typeName}' union member index '${segment.memberIndex}'`;
    default:
      return unreachable(segment["segmentType"]);
  }
}

export function getNdcSchema(
  ...
  const scalarTypes: Record<string, sdk.ScalarType> = mapObjectValues(
    functionsSchema.scalarTypes,
    (scalarDef, scalarTypeName) => {
      switch (scalarDef.type) {
        case "built-in":
          return isTypeNameBuiltInScalar(scalarTypeName)
            ...
            : throwError(
                `built-in scalar type with unexpected name: ${scalarTypeName}`,
              );
        case "relaxed-type":
          return {
            aggregate_functions: {},
  ...
  return {
    functions: functions
      ...
      .map(([name, def]) =>
        convertFunctionDefinitionToSdkSchemaType(name, def),
      ),
...
function convertTypeReferenceToSdkType(typeRef: TypeReference): sdk.Type {
  switch (typeRef.type) {
    ...
    default:
      return unreachable(typeRef["type"]);
  }
}

function convertBuiltInScalarTypeIntoSdkSchemaType(
  typeName: BuiltInScalarTypeName,
): sdk.ScalarType {
  switch (typeName) {
    case BuiltInScalarTypeName.String:
      return {
        representation: { type: "string" },
        aggregate_functions: {},
        comparison_operators: { _eq: { type: "equal" } },
      };
    case BuiltInScalarTypeName.Float:
      return {
        representation: { type: "float64" },
        aggregate_functions: {},
        comparison_operators: { _eq: { type: "equal" } },
      };
    case BuiltInScalarTypeName.Boolean:
      return {
        representation: { type: "boolean" },
        aggregate_functions: {},
        comparison_operators: { _eq: { type: "equal" } },
      };
    case BuiltInScalarTypeName.BigInt:
      return {
        representation: { type: "biginteger" },
        aggregate_functions: {},
        comparison_operators: { _eq: { type: "equal" } },
      };
    case BuiltInScalarTypeName.DateTime:
      return {
        representation: { type: "timestamp" },
        aggregate_functions: {},
        comparison_operators: { _eq: { type: "equal" } },
      };
    case BuiltInScalarTypeName.JSON:
      return {
    ...
    default:
      return unreachable(typeName);
  }
}

function convertFunctionDefinitionToSdkSchemaType(
  function_name: string,
  definition: FunctionDefinition,
): sdk.FunctionInfo | sdk.ProcedureInfo {
  const args = definition.arguments.map((argDef) => [
    argDef.argumentName,
    {
...
export function printSchemaListing(
  functionNdcKind: FunctionNdcKind,
  functionDefinitions: FunctionDefinitions,
) {
  const functions = Object.entries(functionDefinitions).filter(
    ([_, def]) => def.ndcKind === functionNdcKind,
  );
...
export function printRelaxedTypesWarning(functionsSchema: FunctionsSchema) {
  const relaxedTypes = Object.entries(functionsSchema.scalarTypes).flatMap<
    [string, RelaxedScalarTypeDefinition]
  ...
  if (relaxedTypes.length > 0) {
    console.error(
      "The following unsupported types have been defined as custom scalar types with no runtime validation ('relaxed types').",
    );
    console.error(
      "Use of relaxed types may cause runtime issues if invalid values are passed for these types.",
    );
    console.error(
      "It is recommended that you replace them with equivalent supported types so that your schema can be properly defined and validated, then remove the @allowrelaxedtypes tag from the functions involved.",
    );
...
export function isTypeNameBuiltInScalar(
  typeName: string,
): typeName is BuiltInScalarTypeName {
  return (
    Object.values(BuiltInScalarTypeName).find(
      (builtInScalarTypeName) => typeName === builtInScalarTypeName,
    ) !== undefined
  );
}

export function isBuiltInScalarTypeReference(
  typeReference: NamedScalarTypeReference,
): typeReference is BuiltInScalarTypeReference {
